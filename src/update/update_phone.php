<?php
    ...
    if(isset($_POST['id'])){
        ...
        try {
            ...
            // create query
            $sql = "UPDATE user SET phone='$phone', updated_by=".$user->userId.", updated_date=NOW() WHERE user_id=$id";
            $stmt = $db->prepare($sql);
            $stmt->execute();

            // update user info
            $newInfo = json_decode($_SESSION['user'], true); // decode JSON to array
            $newInfo['phone'] = $phone; // update to new profile image
            ...
            echo json_encode(array(
                "status"=>1,
                "data"=>"Updated phone successfully",
            ));
